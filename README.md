...
- PC 优先应用壳：固定导航、查询工作台和收藏资料库
- 当前季度番表、季度前后切换、关键词搜索、筛选、排序、最近搜索
- 高密度结果列表、番名复制、保留上下文的详情面板
- 本地收藏状态、状态统计、库内搜索/排序、JSON 导入/导出
- 内部 API route 占位
...
src/app/components              应用壳与番剧工作台组件
...
src/app/collection              本地收藏页
src/lib/anime                   动画领域模型与季度 helper
src/lib/client                  客户端 API、Radix headless 控件和收藏 hook
...
src/lib/platform                收藏、最近搜索、剪贴板等可替换的平台抽象
e2e                             Playwright 测试
```
...
- UI 以 PC 桌面工具体验为主；交互控件优先使用 Radix UI headless primitives。
